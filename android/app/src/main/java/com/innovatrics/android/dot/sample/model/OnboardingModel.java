...
import com.innovatrics.android.dot.sample.http.dto.DocumentPage;
import com.innovatrics.android.dot.sample.http.dto.Image;
import com.innovatrics.android.dot.sample.http.dto.LivenessPosition;
import com.innovatrics.android.dot.sample.http.dto.MrzField;
...
import com.innovatrics.android.dot.sample.http.dto.ReviewField;
import com.innovatrics.android.dot.sample.http.dto.TextField;
import com.innovatrics.android.dot.sample.http.dto.TextFieldLine;
...
public class OnboardingModel extends AndroidViewModel {
    ...
    private final List<SegmentConfiguration> segmentConfigurationList;
    ...
    public List<SegmentConfiguration> getSegmentConfigurationList() {
    ...
    public void processCapturedDocumentPageInBackground(final byte[] data) {
    ...
    private synchronized void processCapturedDocumentPage(final byte[] data, final DocumentSide documentSide) {
        final Uri uri;
    ...
    private Bitmap cropAndScaleDown(final byte[] data) {
        final Bitmap bitmap = Bitmaps.fromJpeg(data);
    ...
    public void faceCaptureDone(final DetectedFace detectedFace) {
        ...
        com.innovatrics.android.dot.utils.Utils.saveBitmapAsJpeg(bitmap, faceCaptureUri);
    ...
    private void prepareLivenessCheck() {
        webservice.onboardingGenerateLivenessPositions("Bearer " + bearerToken).enqueue(new CallbackImpl<>(new WebserviceCallback<OnboardingGenerateLivenessPositionsResponse>() {
            ...
            public void onResponse(final OnboardingGenerateLivenessPositionsResponse response) {
                ...
                for (final LivenessPosition livenessPosition : response.getPositionList()) {
                    segmentConfigurationList.add(new SegmentConfiguration(livenessPosition.name(), 1000));
                }
    ...
    public void livenessCheckDone(final float score, final List<SegmentPhoto> segmentPhotoList) {
        ...
        for (final SegmentPhoto segmentPhoto : segmentPhotoList) {
            final Bitmap bitmap = segmentPhoto.getPhoto().toBitmap();
            final byte[] image;
